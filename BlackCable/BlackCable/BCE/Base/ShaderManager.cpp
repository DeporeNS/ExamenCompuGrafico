...
namespace BCE
{
	namespace Base
	{
		...
		ShaderManager::ShaderManager(): currentShader(nullptr)
		{
			...
			LoadShaders("gouraud", vShaderGouraud, fShaderGouraud);
			LoadShaders("phong", vShaderPhong, fShaderPhong);
		}
		...
		void ShaderManager::LoadShaders(const std::string & name, const std::string & pathV, const std::string & pathF)
		{
			...
			shader2->CreateFromFiles(pathV.c_str(), pathF.c_str());
			shaderList[name] = (*shader2);

			Shader* shader3 = new Shader();
			shader3->CreateFromFiles(pathV.c_str(), pathF.c_str());
			shaderList[name] = (*shader3);
		}
